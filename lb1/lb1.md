...
- `sum [pollution] of patches` — сумарне забруднення (за потреби).

---

### 6. Алгоритм (циклічна логіка `go`)
1. Кожен такт люди **рухаються**, можуть **народжуватися** та **саджати дерева**, **вбирають забруднення** (що шкодить здоров'ю) і можуть **померти**.  
...
> У кожному експерименті змінюємо лише **один** параметр; решту залишаємо, як у розділі 7. Для кожного значення параметра виконуємо 3–5 прогонів і фіксуємо середні/медіанні значення метрик.

### Експеримент 1. Вплив кількості електростанцій на забруднення і демографію
**Мета:** дослідити залежність `mean [pollution] of patches`, `count people`, `count trees` від **power-plants**.  
...
| power-plants | pollution | people (кін.) | trees (кін.) |
|---:|---:|---:|---:|
| 0 | 0 | 283 | 991 |
| 4 | 6 | 829 | 1763 |
| 8 | 23 | 433 | 1125 |
...
**3)** Існує пороговий ефект: при ~12–20 станціях система входить у зону високого забруднення, де великі популяції людей/дерев стають нестійкими.
...
**Метрики:** mean pollution, people (кін.), trees (кін.).

| polluting-rate | pollution | people (кін.) | trees (кін.) |
|---:|---:|---:|---:|
| 0 | 0 | 205 | 646 |
| 1 | 2 | 220 | 625 |
| 2 | 5.9 | 236 | 694 |
...
**Метрики:** mean pollution (кін.), people (кін.), trees (кін.), а також криві «pollution vs time».

| planting-rate | pollution | people (кін.) | trees (кін.) |
|---:|---:|---:|---:|
| 0.00 | 816 | 0 | 0 |
| 0.02 | 28.5 | 231 | 197 |
| 0.04 | 10 | 502 | 918 |
